...
namespace srouter::rpc
{
    ...
    void load_value(BTConsumer& c, T& target)
    {
        ...
        else if constexpr (srouter::rpc::json_is_binary<T>)
            srouter::rpc::load_binary_parameter(c.consume_string_view(), true /*allow raw*/, target);
    ...
    void load_value(json_range& range_itr, T& target)
    {
        ...
        else if constexpr (std::is_same_v<T, std::string> || std::is_same_v<T, std::string_view>)
        {
            ...
            srouter::rpc::json_is_binary<T> || is_expandable_list<T> || is_tuple_like<T> || is_unordered_string_map<T>)
        {
            try
            {
